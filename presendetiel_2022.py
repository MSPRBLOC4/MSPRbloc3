...
fichier = "data_election_presidentiel/resultats-par-niveau-subcom-t1-france-entiere.xlsx"
orientation_df = pd.read_excel("data_election_presidentiel/parti condidat 2022.xlsx")
...
def meilleur_score(row):
    ...
    for i in range(17):
        ...
        nom_candidat = bloc.iloc[2].strip().title()
        if pd.isna(nom_candidat):
            break
        #code_nuance = bloc.iloc[1]
        prenom_candidat = bloc.iloc[3].strip().title()
        try:
...
df_final.to_csv("presidentielle_2022_final.csv", index=False, encoding="utf-8")
