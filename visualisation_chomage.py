...
# ---------------------------------------------------------
# 2) Agrégation Chômeurs par région + année
# ---------------------------------------------------------

agg = (
    df_final
      .groupby(["annee", "region"], as_index=False)["chomeurs15_24ans"]
      .sum()
)
...
top5_by_year = (
    agg
      .sort_values(["annee","chomeurs15_24ans"], ascending=[True, False])
      .groupby("annee")
      .head(5)
)

# 4) Pivot pour avoir en colonnes chaque année
pivot = top5_by_year.pivot(index="region", columns="annee", values="chomeurs15_24ans").fillna(0)
...
ax.set_title("Top 5 régions par nombre de Chômeurs (2017 vs 2022)")
ax.set_ylabel("Nombre de Chômeurs")
...
# 1) Somme des chomeurs15_24ans par année
agg = df_final.groupby('annee', as_index=False)['chomeurs15_24ans'].sum()

years  = agg['annee'].astype(str)
values = agg['chomeurs15_24ans']
...
# 7) Titres et style
ax.set_title("Comparaison du nombre de Chômeurs\n2017 vs 2022", pad=15)
ax.set_xlabel("Année")
...
agg = df_bret.groupby('annee', as_index=False)['chomeurs15_24ans'].sum()
...
# 7) Titres et style
ax.set_title("Comparaison du nombre de Chômeurs En Bretagne\n2017 vs 2022", pad=15)
ax.set_xlabel("Année")
ax.set_ylabel("Nombre Chômeurs")
plt.ylim(0, values.max()*1.15)
plt.tight_layout()
plt.show()