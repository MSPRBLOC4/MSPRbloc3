...
agg = (
    df_final
      .groupby(["annee", "region"], as_index=False)["NB"]
      .sum()
)
...
top5_by_year = (
    agg
      .sort_values(["annee","NB"], ascending=[True, False])
      .groupby("annee")
      .head(5)
)

# 4) Pivot pour avoir en colonnes chaque année
pivot = top5_by_year.pivot(index="region", columns="annee", values="NB").fillna(0)
...
ax.set_title("Top 5 régions par nombre d'immigrés (2017 vs 2022)")
...
# 1) Somme des NB par année
agg = df_final.groupby('annee', as_index=False)['NB'].sum()
...
# 7) Titres et style
ax.set_title("Comparaison du nombre d'immigrés\n2017 vs 2022", pad=15)
ax.set_xlabel("Année")
ax.set_ylabel("Nombre d'immigrés")
...
agg = df_bret.groupby('annee', as_index=False)['NB'].sum()
...
# 7) Titres et style
ax.set_title("Comparaison du nombre d'immigrés En Bretagne\n2017 vs 2022", pad=15)
ax.set_xlabel("Année")
ax.set_ylabel("Nombre d'immigrés")
plt.ylim(0, values.max()*1.15)
plt.tight_layout()
